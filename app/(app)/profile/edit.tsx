...
export default function EditProfileScreen() {
  ...
  return <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.screen}>
    ...
    <View style={styles.footer}><TouchableOpacity disabled={loading} onPress={handleUpdate} style={styles.saveButton}><Text style={styles.saveButtonText}>{loading ? "Saving..." : "Save Changes"}</Text></TouchableOpacity></View>
...
const styles = StyleSheet.create({
  screen: { flex: 1, backgroundColor: "#000000" }, safeArea: { flex: 1, paddingTop: Platform.OS === "android" ? (StatusBar.currentHeight || 0) + 8 : 8 }, content: { paddingHorizontal: 15, paddingBottom: 12 },
  ...
  accountCard: { borderRadius: 18, backgroundColor: "#171717", borderWidth: 1, borderColor: "#303030", paddingHorizontal: 15, position: "relative" }, accountRow: { flexDirection: "row", gap: 20 }, accountInput: { flex: 1, gap: 4 }, usernameInput: { marginTop: 1, gap: 4 }, sparkle: { position: "absolute", right: 14, bottom: 15, transform: [{ rotate: "20deg" }] }, footerSpacer: { height: 68 }, footer: { position: "absolute", left: 15, right: 15, bottom: Platform.OS === "ios" ? 8 : 10 }, saveButton: { minHeight: 49, borderRadius: 16, alignItems: "center", justifyContent: "center", backgroundColor: "#10C463" }, saveButtonText: { color: "#DDFBEA", fontSize: 17, fontWeight: "500" },
